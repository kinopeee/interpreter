...
- 原文音声は専用 `gpt-live-transcribe` へ常時送信する。判定前は原文のみ＋直近2秒のprerollを保持し、判定後は日本語なら `target=en`、英語なら `target=ja` へprerollから送る。
...
- `Audio/SpokenLanguage.swift`: 原文deltaの文字種判定（補助信号）。
...
- WebSocket送信はactor境界で管理する。原文送信は翻訳送信と分離し、翻訳側の停滞・失敗に巻き込まない。判定後は選択されたtargetだけが同じ音声frame列をprerollから受信する。
...
- lane選択の一次信号は「どちらのセッションが `output_transcript.delta` を出したか」とする。出力言語と同じ言語の音声は翻訳されない（same-language抑制）。
- 原文deltaの文字種判定は補助信号として使う。
...
- 最低限、イベントcodec、100 ms packet化、専用原文transcription、原文送信分離とpreroll、送信timeout、字幕lane選択、旧epoch破棄、停止時close drainを維持する。
